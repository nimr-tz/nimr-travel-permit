...
<div x-data="{
    ...
    <aside
        ...
        <nav class="flex-1 overflow-y-auto py-5 px-3 space-y-1.5">
            ...
            @php $aActive = request()->routeIs('audit-log.*'); @endphp
            <a href="{{ route('audit-log.index') }}" title="{{ __('nav.audit_log') }}"
               class="group flex items-center gap-3 px-3 py-3 rounded-lg text-sm transition-all
                      {{ $aActive ? 'text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}"
               style="{{ $aActive ? 'background-color:#05499c;' : '' }}">
                <span class="shrink-0 w-5 h-5">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                </span>
                <span x-show="!collapsed">{{ __('nav.audit_log') }}</span>
            </a>
            @php $spActive = request()->routeIs('security-policy.*'); @endphp
            <a href="{{ route('security-policy.index') }}" title="{{ __('nav.security_policy') }}"
               class="group flex items-center gap-3 px-3 py-3 rounded-lg text-sm transition-all
                      {{ $spActive ? 'text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}"
               style="{{ $spActive ? 'background-color:#05499c;' : '' }}">
                <span class="shrink-0 w-5 h-5">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                </span>
                <span x-show="!collapsed">{{ __('nav.security_policy') }}</span>
            </a>
            @endif
        </nav>
